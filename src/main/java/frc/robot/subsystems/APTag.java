...
public class APTag extends SubsystemBase {
  ...
  public boolean atPositionX(double tx, double deadzone){
    boolean atPos = false;
    if((this.getX() >= tx-deadzone && this.getX() <= tx+deadzone)){
      atPos = true;
    }
    return atPos;
  }

  public boolean atPositionY(double ta, double deadzone){
    boolean atPos = false;
    if((this.getA() >= ta-deadzone && this.getA() <= ta+deadzone)){
      atPos = true;
    }
    return atPos;
  }

  public boolean atPositionXY(double tx, double ta, double deadzone){
    boolean atPos = false;
    if((this.getX() >= tx-deadzone && this.getX() <= tx+deadzone) && (this.getA() >= ta-deadzone && this.getA() <= ta+deadzone)){
      atPos = true;
    }
    return atPos;
  }

  public boolean atRotation(double currentYaw, double targetYaw, double deadzone){
    if(currentYaw < targetYaw - deadzone || currentYaw > targetYaw + deadzone){
      return true;
    }
    else{
      return false;
